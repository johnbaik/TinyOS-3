...
#define numOfQueues 101
#define max_num_of_yields 1500
...
TCB* spawn_thread(PCB* pcb, void (*func)())
{
  ...
  /* Set the owner */
  tcb->owner_pcb = pcb;
  //tcb->owner_ptcb->ptcb_node=pcb->headPtcb;
...
rlnode SCHED[numOfQueues];                         /* The scheduler queue */
...
void give_priority(enum SCHED_CAUSE cause,TCB* tcb){
...
void checkPriorityInversion(TCB* tcb){

	if(tcb->cause==SCHED_MUTEX && tcb->previous_Cause==SCHED_MUTEX){
 ...
 void checkStarvation(){
	 ...
	 for(int i=1;i<numOfQueues;i++){
		  tmp=&SCHED[i];
		  if((&SCHED[i].node==NULL)){
		 for(int j=0;j<rlist_len(&SCHED[i]);j++){
			 tmp->tcb->numOfYields++;
					 if(tmp->tcb->numOfYields>=max_num_of_yields){
						 rlist_push_back(&SCHED[0],rlist_remove(tmp->node));
						 tmp->tcb->numOfYields=0;
					 }
					 tmp=tmp->next;
		 	 	 	 }
...
{ 
  ...
  current->previous_Cause=current->cause;
  current->cause=cause;
   checkPriorityInversion(current);
   give_priority(cause,current);
   checkStarvation();
